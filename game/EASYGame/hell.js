...
//animation frames
(function() {
    var requestAnimationFrame = window.requestAnimationFrame || window.mozRequestAnimationFrame || window.webkitRequestAnimationFrame || window.
...
function update() {
    ...
    for (var i = 0; i < terrain.length; i++) {
        ...
        //evil-bullet-terrain collision
        var dtr = colCheck(evilBullet2, terrain[i]);
        if (dtr != null){
        	sniper2.shot = false;
        }
    ...
    for (var i = 0; i < bumpers.length; i++) {
        ...
        var diy = colCheck(enemy4, bumpers[i]);
        if (diy === "l") {
    ...
    if(sniper2.shot){
    	evilBullet2.x -= evilBullet2.speed;
    }
    //Player Actions
    if(player.grounded){
         player.velY = 0;
